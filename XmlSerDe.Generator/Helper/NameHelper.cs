...
namespace XmlSerDe.Generator.Helper
{
    public static class NameHelper
    {
        ...
        private static readonly SymbolDisplayFormat _reflectionFormatWithoutNamespace = (SymbolDisplayFormat)_constructor.Invoke(
            new object[]
            {
                        _symbolDisplayCompilerInternalOptions_UseArityForGenericTypes,
                        SymbolDisplayGlobalNamespaceStyle.Omitted, //default(SymbolDisplayGlobalNamespaceStyle),
                        SymbolDisplayTypeQualificationStyle.NameAndContainingTypes, //default(SymbolDisplayTypeQualificationStyle),
                        SymbolDisplayGenericsOptions.IncludeTypeParameters, //default(SymbolDisplayGenericsOptions),
                        default(SymbolDisplayMemberOptions),
                        default(SymbolDisplayDelegateStyle),
            ...
            )
        {
            ...
            var reflectionTypeString = s.ToDisplayString(includeNamespace ? _reflectionFormat : _reflectionFormatWithoutNamespace);
            if (s.TypeArguments.Length > 0)
            {
                ...
                foreach (var gta in s.TypeArguments)
                {
                    ...
                    {
                        throw new Exception($"[{gta.ToDisplayString()}] is not a {nameof(INamedTypeSymbol)}");
                    }
