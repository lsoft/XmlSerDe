...
namespace XmlSerDe.Generator
{
    ...
    public class XmlDeserializeGenerator : IIncrementalGenerator
    {
            ...
            )
        {
            ...
            {
                //у отказа с названной причиной стектрейс только мешает: читать
                //его некому, а сообщение он топит
                var msg = (
                        excp is GenerationRefusedException
                            ? excp.Message
        ...
        internal sealed class DocumentAdder
        {
                ...
                )
            {
                ...
                _sources.Add(
                    new GeneratedSource(
                        documentName,
                        GeneratedFilePrelude + BlankOutWhitespaceOnlyLines(document)
                        )
                    );
            }

            /// <summary>
            /// Шапка, с которой начинается каждый сгенерированный файл.
            ///
            /// Сгенерированный код компилируется в сборке потребителя и с его
            /// настройками предупреждений, а починить в нём потребитель ничего не
            /// может: файла у него нет. Замерено на пустом проекте:
            /// <c>GenerateDocumentationFile</c> даёт 654 CS1591, алиас
            /// <c>roschar</c> - 20 CS8981, а с <c>TreatWarningsAsErrors</c> любое из
            /// этих чисел означает, что проект не собирается вовсе.
            ///
            /// Поэтому глушится всё, а не перечисленное поимённо: список закрывает
            /// те предупреждения, про которые мы догадались, а потребитель включает
            /// те, про которые не догадались. Свой сигнал при этом не теряется -
            /// <c>GeneratedPreludeFixture</c> компилирует тот же код со снятой шапкой
            /// и требует, чтобы предупреждений не было ни одного, кроме названных
            /// там поимённо.
            ///
            /// <c>&lt;auto-generated/&gt;</c> в первой строке - отдельная просьба,
            /// к другому адресату: анализаторы и IDE-подсказки смотрят на неё, а не
            /// на pragma.
            /// </summary>
            internal static readonly string GeneratedFilePrelude =
                "// <auto-generated/>" + Environment.NewLine
                + "#pragma warning disable" + Environment.NewLine;
