...
| `BclPort/*` | Shapes ported from the `System.Xml.Serialization` test suite in dotnet/runtime (`System.Private.Xml/tests/XmlSerializer`). 33 of them run through the same differential harness as `Interop/*` and are reported in `bcl-port-report.md`; 22 are fully compatible and the rest pin a divergence each - `[DefaultValue]` on a `char`, `ShouldSerializeXxx()`, `[XmlArray(IsNullable = true)]`, a `null` item of a `Nullable<T>` collection, `[XmlElement(Order = n)]` on read when two members share an element name, `DataType =` (`hexBinary` and `time`), a `[Flags]` combination, and member order within one type (the BCL writes every field before every property; XmlSerDe writes them in declaration order) |
| `BclPort/BclPortReadFixture` | The ported tests whose input is a document rather than an object, so a round-trip cannot express them: CDATA surrounded by indentation (where the two sides disagree - the BCL drops the adjacent whitespace-only text node, XmlSerDe keeps it), an empty element on a `TimeSpan`, a setter-less property left to its constructor value, an unknown element skipped with its subtree, an out-of-range `byte`, and ` ` in a string - refused on write and on read by both sides |
| `BclPort/BclGeneratedShapeFixture` | Ported shapes driven through `CSharpGeneratorDriver` so that generator output which does not compile is one red test rather than a red project - in a host, a shape like this kills generation for every host at once. Covers the `[DefaultValue]` values that used to do exactly that (`NaN`, `±Infinity`, an `int` for an enum member, a `bool` for an `int` member, the `(Type, string)` overload), and `[XmlElement]` on a collection member, which must be refused by a diagnostic that names the member and carries no stack trace |
